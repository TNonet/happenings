...
for day_of_week in [
    ...
    DAYS_OF_WEEK[day_of_week] = function_event(_lambda(day_of_week), vectorized=True, name=day_of_week, dtype=np.bool)
...
US_FEDERAL_HOLIDAYS["New Year's Day"] = function_event(
    lambda x: x.month == 1 and x.day == 1, vectorized=True, name="New Year's Day", dtype=np.bool,
)
...
US_FEDERAL_HOLIDAYS["Independence Day"] = function_event(
    lambda x: (x.month == 7) & (x.day == 4), vectorized=True, name="Independence Day", dtype=np.bool,
)
...
US_FEDERAL_HOLIDAYS["Veterans Day"] = function_event(
    lambda x: (x.month == 11) & (x.day == 11), vectorized=True, name="Veterans Day", dtype=np.bool,
)
...
US_FEDERAL_HOLIDAYS["Christmas Day"] = function_event(
    lambda x: (x.month == 12) & (x.day == 25), vectorized=True, name="Christmas Day", dtype=np.bool,
)
...
US_OTHER_HOLIDAYS["Halloween"] = function_event(
    lambda x: (x.month == 10) & (x.day == 31), vectorized=False, name="Halloween", dtype=np.bool,
)

US_OTHER_HOLIDAYS["Valentine's Day"] = function_event(
    lambda x: (x.month == 2) & (x.day == 14), vectorized=False, name="Valentine's Day", dtype=np.bool,
)

US_OTHER_HOLIDAYS["Saint Patrick's Day"] = function_event(
    lambda x: (x.month == 3) & (x.day == 17), vectorized=False, name="Valentine's Day", dtype=np.bool,
)
