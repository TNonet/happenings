...
from evendar.event_calendar import week_of_year
...
for index, event_name in enumerate([
    ...
    MONTHS[event_name] = function_event(_lambda(index),
                                        vectorized=True,
                                        name=event_name,
                                        dtype=np.bool)

DAY = function_event(lambda x: x.day,
                     vectorized=True,
                     name='Day',
                     dtype=np.bool)
...
for day_of_week in [
    ...
    DAYS_OF_WEEK[day_of_week] = function_event(_lambda(day_of_week),
                                               vectorized=True,
                                               name=day_of_week,
                                               dtype=np.bool)
    ...
    def _lambda(i):
        return i
    WEEK_OF_YEAR[day_of_week] = function_event(partial(week_of_year, week_start_day=day_of_week),
                                               vectorized=True,
                                               name=f"W-{day_of_week}",
                                               dtype=str)
...
US_FEDERAL_HOLIDAYS["New Year's Day"] = function_event(lambda x: x.month == 1 and x.day == 1,
                                                       vectorized=True,
                                                       name="New Year's Day",
                                                       dtype=np.bool)
